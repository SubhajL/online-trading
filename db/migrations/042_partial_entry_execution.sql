-- 042_partial_entry_execution.sql: durable cumulative entry execution and
-- crash-safe ownership for deferred entry finalization.

ALTER TABLE bracket_legs
    ADD COLUMN IF NOT EXISTS executed_quantity NUMERIC(18,8) NOT NULL DEFAULT 0;

UPDATE bracket_legs
SET executed_quantity = 0
WHERE executed_quantity IS NULL;

ALTER TABLE bracket_legs
    ALTER COLUMN executed_quantity SET DEFAULT 0,
    ALTER COLUMN executed_quantity SET NOT NULL;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'bracket_legs'::regclass
          AND conname = 'chk_bracket_leg_executed_quantity'
    ) THEN
        ALTER TABLE bracket_legs
            ADD CONSTRAINT chk_bracket_leg_executed_quantity
...
ALTER TABLE brackets
    ADD COLUMN IF NOT EXISTS entry_finalization_lease_token UUID,
    ADD COLUMN IF NOT EXISTS entry_finalization_lease_until TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS idx_brackets_entry_finalization_lease
    ON brackets (entry_finalization_lease_until)
    WHERE status IN ('RESERVED', 'ENTRY_PLACED', 'ENTRY_FILLED');
...
AFTER UPDATE OF status, exchange_order_id, average_fill_price, executed_quantity
ON bracket_legs
FOR EACH ROW
WHEN (
    ...
    OR OLD.executed_quantity IS DISTINCT FROM NEW.executed_quantity
)
