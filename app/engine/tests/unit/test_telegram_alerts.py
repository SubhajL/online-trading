...
@pytest.fixture
def telegram_adapter(mock_event_bus):
...
async def test_handle_decision_with_snapshot(telegram_adapter):
    ...
    # Mock helper methods
    telegram_adapter._get_snapshot_url = AsyncMock(return_value="http://example.com/snapshot.png")
    ...
    telegram_adapter._download_snapshot.assert_called_once_with("http://example.com/snapshot.png")
...
async def test_get_snapshot_url_success(telegram_adapter):
    ...
    mock_response = MagicMock()
    mock_response.status = 200
    mock_response.json = AsyncMock(return_value={"imageUrl": "http://example.com/snap.png"})
    ...
    with patch.dict('os.environ', {'BFF_URL': 'http://bff:3000', 'INTERNAL_API_KEY': 'test-key'}):
