...
def run_mypy_on_engine() -> tuple[bool, list[MypyError]]:
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        cwd='/Users/subhajlimanond/dev/online trader'
    )
...
class TestMypyProgress:
    ...
    def test_missing_any_imports(self) -> None:
        ...
        for file, lines in sorted(files_needing_any.items())[:5]:
            print(f"  {file}: lines {sorted(set(lines))[:3]}...")
    ...
    def test_type_annotation_errors(self) -> None:
        ...
        # Group by pattern
        conn_annotations = [e for e in annotation_errors if 'Need type annotation for "conn"' in e.message]

        print(f"\nMissing type annotations: {len(annotation_errors)}")
        print(f"  Connection objects: {len(conn_annotations)}")

        # Currently expecting annotation errors
        assert len(annotation_errors) > 0  # Will change to == 0 when fixed
    ...
    def test_error_categories(self) -> None:
        ...
        # Currently expecting errors
        assert not success
        assert len(errors) > 0

        # Most common should be name-defined (missing imports)
        assert 'name-defined' in categories
