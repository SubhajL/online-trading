...
def get_migration_config() -> Dict[str, Any]:
    ...
    return {
        "timeout": float(os.getenv("MIGRATION_TIMEOUT", "120000")) / 1000,  # ms to seconds
...
def record_migration_metric(metrics: Dict[str, Any], operation: str, success: bool) -> None:
    ...
    if success:
        metrics["migration_success_total"]["value"] = metrics.get("migration_success_total", {}).get("value", 0) + 1
    else:
        metrics["migration_failure_total"]["value"] = metrics.get("migration_failure_total", {}).get("value", 0) + 1
