...
class BinanceUSDMIngester:
    ...
    - Enhanced time sync error handling with recvWindow adjustment
    """
    ...
    async def _connect_and_subscribe(self) -> None:
        """Connect to WebSocket and subscribe to futures kline streams."""
    ...
    async def _on_reconnect(self) -> None:
        ...
        logger.info("Starting futures backfill after reconnection...")
            ...
            for tf in self.timeframes:
                ...
                if not last_time:
                    ...
                    latest_candle = await self.db_adapter.get_latest_candle(
                        symbol=symbol, timeframe=TimeFrame(tf)
                    )
                    if latest_candle:
    ...
    async def _backfill_missing_candles(
        self, symbol: str, timeframe: str, start_time: datetime, retry_count: int = 0
    ) -> None:
        """
        ...
        async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(url) as response:
                        ...
                        for kline in klines:
                            candle = rest_kline_to_candle(
                                kline, symbol, timeframe, self.venue
                            )
            ...
            if self._time_sync_errors == 0:
                self.recv_window = 5000  # Reset to default

    async def _handle_time_sync_error(self, error_msg: str) -> None:
        """
        Handle Binance -1021 time sync errors with dynamic recvWindow adjustment.
