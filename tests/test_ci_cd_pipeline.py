...
def test_ci_workflow_has_essential_jobs():
    ...
    for job_type in essential_job_types:
        assert any(
            job_type in job_name.lower() for job_name in job_names
...
def test_cd_workflow_structure():
    ...
    assert (
        "CD" in config["name"]
        or "Deploy" in config["name"]
        or "Continuous Deployment" in config["name"]
    )
    ...
    assert (
        "tags:" in content or "release:" in content or "workflow_dispatch:" in content
    )
